      ...
      <AnimatePresence>
        ...
        {isMenuOpen && (
          <motion.div
            ...
            className="fixed top-20 right-0 w-[85%] md:w-[340px] bottom-0 bg-white z-40 shadow-2xl flex flex-col border-l border-stone-200 overflow-y-auto"
          >
            {/* Close button */}
            ...
            {/* Nav Links */}
            <div className="flex flex-col flex-1 px-7 pb-10 space-y-1 w-full">
                ...
                <div key={section.label} className="border-b border-stone-100">
                  {section.children.length > 0 ? (
                    <>
                      ...
                      <AnimatePresence initial={false}>
                        {openSection === section.label && (
                          ...
                          >
                            <div className="flex flex-col space-y-3 pb-5 pl-4">
                              {section.children.map((child) => (
                                <Link
                                  key={child.href}
                                  href={child.href}
                                  onClick={() => setIsMenuOpen(false)}
                                  className="text-sm uppercase tracking-[0.25em] font-light text-stone-500 hover:text-[#D4AF37] transition-colors"
                                >
                                  {child.label}
                                </Link>
                              ))}
                    ...
                    <Link
                      href={section.href}
                      onClick={() => setIsMenuOpen(false)}
                      className="block py-4 text-lg uppercase tracking-widest font-extralight text-stone-900 hover:text-[#D4AF37] transition-colors"
                    >
                      {section.label}
                    </Link>
                  )}
